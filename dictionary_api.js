...
const FD_STATUS_CACHE =
  typeof window !== "undefined" && window.__FDStatusCache
...
const FD_STATUS_LRU =
  typeof window !== "undefined" && window.__FDStatusLRU
    ? window.__FDStatusLRU
    : Object.create(null);
let FD_STATUS_SEQ =
  (typeof window !== "undefined" && window.__FDStatusSeq) || 0;

const FD_STATUS_TS =
  typeof window !== "undefined" && window.__FDStatusTS
    ? window.__FDStatusTS
    : Object.create(null);
...
function persistIndependentFDCache(limit = 2000) {
  try {
    ...
    const entries = {};
    trimmed.forEach(([k, seq]) => {
      entries[k] = { v: FD_STATUS_CACHE[k], s: seq, t: FD_STATUS_TS[k] || now };
    });
...
function touchFD(key) {
  ...
  if (
    ...
    typeof window.__FDStatusSeq === "number" &&
...
function maybePruneFD(limit = 2000, buffer = 50) {
  try {
    const keys = Object.keys(FD_STATUS_CACHE).filter(
      (k) => typeof FD_STATUS_CACHE[k] === "boolean"
    );
...
async function validateWordAPIBatch(displayWords) {
  ...
  const sharedCache =
    typeof window !== "undefined" && window.__FDStatusCache
      ? window.__FDStatusCache
      : (globalThis.__FDStatusCache ||= Object.create(null));
  ...
  for (const pw of plainList) {
    ...
    if (typeof v === "boolean" ? v : true) validPlain.add(pw);
    else invalidPlain.add(pw);
  }
...
function htmlForEntry(entry, { senseLimit = 3, entryIdx = 0 } = {}) {
  ...
  const header = `
    <div class="flex items-center gap-2">
      ...
      ${
        phonetic
          ? `<span class="text-sm text-gray-500">${esc(phonetic)}</span>`
          : ""
      }
      ${
        audioUrl
  ...
  const blocks = meanings
    .map((m, mi) => {
      ...
      const sensesHtml =
        show.map((s, i) => senseLine(s, i)).join("") +
        (hide.length
          ? `
      ...
      return `
      <div class="mt-2">
        <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-gray-100 text-gray-700 text-xs uppercase tracking-wide">${esc(
          m.pos
        )}</span>
  ...
  const sources = sourceUrls?.length
    ? `<div class="mt-2 text-[11px] text-gray-400">Source: ${sourceUrls
        .map(
          (u) =>
            `<a href="${esc(u)}" target="_blank" class="hover:underline">${esc(
              u
            )}</a>`
        )
        .join(", ")}</div>`
...
function senseLine(sense, idx) {
  const ex = sense.example
    ? `<div class="text-[12px] text-gray-500 mt-0.5">“${esc(
        sense.example
      )}”</div>`
    : "";
  const synChips = (sense.synonyms || [])
    .slice(0, 8)
    .map((w) => chip(w, false));
  const antChips = (sense.antonyms || []).slice(0, 8).map((w) => chip(w, true));
  const rel = [...synChips, ...antChips];
  const relHtml = rel.length
    ? `<div class="flex flex-wrap items-center gap-1 mt-1"><span class="text-[10px] text-gray-500 mr-1">Synonyms/Antonyms:</span>${rel.join(
        ""
      )}</div>`
    : "";
  return `
...
function renderOnlineDict(word, apiJson, { senseLimit = 3 } = {}) {
  ...
  const html = entries
    .map((e, i) => htmlForEntry(e, { senseLimit, entryIdx: i }))
...
if (typeof window !== "undefined")
  window.getFDCacheStats = function () {
    const now = Date.now();
    const words = Object.keys(FD_STATUS_CACHE).filter(
      (k) => typeof FD_STATUS_CACHE[k] === "boolean"
    );
    let pos = 0,
      neg = 0,
