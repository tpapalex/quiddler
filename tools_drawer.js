...
function initToolsDrawer() {
  ...
  (function registerDictValidation() {
    ...
    function validateDictWord(val) {
      ...
      if (!/^[A-Za-z]+$/.test(v))
        return { status: "error", message: "Letters A-Z only" };
      return { status: "ok" };
    }
    function doRegister() {
      try {
  ...
  async function renderDefinition(word) {
    ...
    if (!cleaned) {
      ...
      try {
        if (!raw) localStorage.removeItem(LAST_DICT_WORD_KEY);
      } catch (_) {}
      try {
        localStorage.removeItem(LAST_DICT_API_JSON_KEY);
      } catch (_) {}
    ...
    try {
      localStorage.setItem(LAST_DICT_WORD_KEY, cleaned);
    } catch (_) {}
    ...
    if (localWrap && localEl) {
      ...
      if (isLocalCollapsed) {
        ...
        window.__fetchLocalDefinition
          ? window.__fetchLocalDefinition(cleaned)
          : null;
      }
    }
        ...
        try {
          ...
          if (rawCache) {
            ...
            if (
              parsed &&
              parsed.w &&
              parsed.w.toLowerCase() === cleaned.toLowerCase() &&
              parsed.d
            ) {
              // Render from cache
              try {
                renderOnlineDict(cleaned, parsed.d, { senseLimit: 3 });
                usedCache = true;
              } catch (_) {}
              // Mark lazy state done
              if (window.__dictLazy && window.__dictLazy.online) {
                window.__dictLazy.online.done = true;
                window.__dictLazy.online.word = cleaned;
              }
            }
          }
        } catch (_) {}
        if (!usedCache) {
          window.__fetchOnlineDefinition
            ? window.__fetchOnlineDefinition(cleaned)
            : null;
        }
      }
    }
  ...
  try {
    persistedState = JSON.parse(
      localStorage.getItem(DICT_SECTION_STATE_KEY) || "null"
    );
  } catch (_) {}
  const dictSectionState = {
    local:
      persistedState &&
      (persistedState.local === "collapsed" ||
        persistedState.local === "expanded")
        ? persistedState.local
        : "expanded",
    online:
      persistedState &&
      (persistedState.online === "collapsed" ||
        persistedState.online === "expanded")
  ...
  window.__dictLazy = {
    local: { word: "", done: false },
    online: { word: "", done: false },
  };
  ...
  window.__fetchLocalDefinition = function (word) {
    const lazy = window.__dictLazy?.local;
    if (!lazy) return;
    lazy.word = word;
    lazy.done = false;
    ...
    let html =
      '<span class="text-gray-500 mt-2 block">No definition found.</span>';
    if (raw) {
      try {
        const parsed =
          typeof parseCollinsEntry === "function"
            ? parseCollinsEntry(word)
            : window.CollinsParsing?.parseCollinsEntry?.(word);
        if (parsed && typeof renderParsedCollins === "function")
          html = renderParsedCollins(parsed);
        else html = raw;
      } catch (_) {
        html = raw;
      }
  ...
  window.__fetchOnlineDefinition = async function (word) {
    const lazy = window.__dictLazy?.online;
    if (!lazy) return;
    lazy.word = word;
    lazy.done = false;
    ...
    if (window.__dictOnlineAbort) {
      try {
        window.__dictOnlineAbort.abort();
      } catch (_) {}
    }
    let abortCtrl = null;
    ...
    try {
      ...
      const apiRes = abortCtrl
        ? await apiFn(word, { signal: abortCtrl.signal })
        : await apiFn(word);
      ...
      if (!error && found && data) {
        ...
        try {
          localStorage.setItem(
            LAST_DICT_API_JSON_KEY,
            JSON.stringify({ w: word, d: data })
          );
        } catch (_) {}
      } else {
        el.innerHTML =
      ...
      if (window.__dictOnlineAbort === abortCtrl)
        window.__dictOnlineAbort = null;
    }
  };
  ...
  function toggleSection(kind) {
    const wrap =
      kind === "local"
        ? document.getElementById("dictLocalWrap")
        : document.getElementById("dictOnlineWrap");
    const bodyEl =
      kind === "local"
        ? document.getElementById("dictLocal")
        : document.getElementById("dictOnline");
    const loadingEl =
      kind === "online" ? document.getElementById("dictOnlineLoading") : null;
    const btn =
      kind === "local"
        ? document.getElementById("dictLocalToggle")
        : document.getElementById("dictOnlineToggle");
    ...
    try {
      localStorage.setItem(
        DICT_SECTION_STATE_KEY,
        JSON.stringify(dictSectionState)
      );
    } catch (_) {}
    if (next === "collapsed") {
      ...
      if (
        kind === "online" &&
        document.getElementById("dictOnline")?.innerHTML.trim() === ""
      ) {
      ...
      if (
        window.__dictLazy &&
        !window.__dictLazy[kind].done &&
        window.__dictLazy[kind].word
      ) {
        if (kind === "local")
          window.__fetchLocalDefinition(window.__dictLazy.local.word);
  ...
  document
    .getElementById("dictLocalToggle")
    ?.addEventListener("click", () => toggleSection("local"));
  document
    .getElementById("dictOnlineToggle")
    ?.addEventListener("click", () => toggleSection("online"));
  ...
  dictInput.addEventListener("input", () => {
    ...
    if (dictDebounceTimer) {
      clearTimeout(dictDebounceTimer);
      dictDebounceTimer = null;
    }
    if (dictBurstTimer) {
      clearTimeout(dictBurstTimer);
      dictBurstTimer = null;
    }
    if (!val.trim()) {
      renderDefinition("");
  ...
  dictInput.addEventListener("keydown", async (e) => {
    if (e.key === "Enter") {
      ...
      const same =
        val && val.toLowerCase() === (__lastLookedUpWord || "").toLowerCase();
      if (same && !dictDebounceTimer && !dictBurstTimer) {
        e.preventDefault();
        return;
      }
      if (dictDebounceTimer) {
  ...
  function applyApiFilterDefault() {
    ...
    try {
      const ds =
        window.QuiddlerGame && window.QuiddlerGame.dictSource
          ? window.QuiddlerGame.dictSource
          : typeof dictSource !== "undefined"
          ? dictSource
          : "local";
  ...
  if (optApiFilter) {
    optApiFilter.addEventListener("change", () => {
      optApiFilter.dataset.userSet = "1";
    });
  }
  ...
  (function registerSolverTilesValidation() {
    ...
    function validateSolverRack(text) {
      ...
      let res = baseFn
        ? baseFn(trimmed, { suppressSingleDigraphWarning: true })
        : { status: "ok" }; // no expectedCards context
      ...
      if (res.status === "ok") {
        try {
          ...
          if (cardCount > 15)
            res = {
              status: "warning",
              message: "Large rack size. Solver may time out",
            };
        } catch {}
      }
  ...
  if (playReset) {
    playReset.addEventListener("click", () => {
      if (tilesInput) {
        tilesInput.value = "";
        try {
          window.InputValidation?.validateElement?.(tilesInput);
        } catch (_) {}
      }
  ...
  async function onFindBestPlay() {
    ...
    try {
      ...
      const result = await Promise.race([
        window.QuiddlerSolver.optimize(tiles, {
          noDiscard,
          currentLongest,
          currentMost,
          apiFilter,
      ...
      if (timedOut) {
        playResult.innerHTML =
          '<div class="text-sm text-red-600">Solver took too long. Try a smaller rack.</div>';
        ...
        playResult.innerHTML =
          '<div class="text-sm text-red-600">No playable words found.</div>';
      }
    } catch (err) {
      console.error(err);
      if (playGo) {
      ...
      playResult.innerHTML =
        '<div class="text-sm text-red-600">Error computing best play.</div>';
    }
  }
  ...
  window.QuiddlerTools = {
    ...
    showDict: async (word) => {
      ...
      showTab("dict");
      if (word) {
        // only overwrite if a new word provided
    ...
    prefillPlay: ({ tiles, currentLongest, currentMost }) => {
      ...
      if (optCurrentLongest && Number.isFinite(currentLongest))
        optCurrentLongest.value = String(currentLongest);
      if (optCurrentMost && Number.isFinite(currentMost))
        optCurrentMost.value = String(currentMost);
      ...
      const longestBonusEnabled =
        document.getElementById("longestWordBonus")?.checked;
      const mostBonusEnabled =
        document.getElementById("mostWordsBonus")?.checked;
      if (!longestBonusEnabled && optCurrentLongest)
        optCurrentLongest.value = "0";
