...
function renderChit(word, opts = {}) {
  ...
  if (word.challenger) {
    const chall = String(word.challenger)
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")
      .replace(/"/g, "&quot;");
  ...
  const wantDefIcon =
    (forceShowDefIcon || effectiveState === "valid") && showDefIcon;
  const defIcon = wantDefIcon
    ? `<span class="ml-1 inline-flex items-center justify-center w-4 h-4 rounded-full bg-gray-500 hover:bg-gray-600 text-white cursor-pointer def-open"
             ...
             title="">${
               window.QuiddlerIcons && window.QuiddlerIcons.img
                 ? window.QuiddlerIcons.img(
                     "dictionary",
                     "w-2.5 h-2.5 filter invert"
                   )
                 : '<img src="icons/magnifying-glass.svg" alt="" class="w-2.5 h-2.5 filter invert"/>'
             }</span>`
    : "";
...
function renderPlayerRowHeader(player, pdata, round) {
  ...
  const dealerEmoji =
    typeof window !== "undefined" && window.QuiddlerGame?.DEALER_EMOJI
      ? window.QuiddlerGame.DEALER_EMOJI
      : "🃏";
  // Smaller emoji (0.85em) for historical rounds only (current round inputs use game.js markup unchanged)
  const nameHTML = `${player}${
    isDealer
      ? `<span class=\"dealer-indicator ml-1.5 align-middle\" style=\"font-size:0.85em; line-height:1; display:inline-block; transform:translateY(-1px);\" aria-label=\"${player} dealt round ${round.roundNum}\" data-tippy-content=\"${player} dealt round ${round.roundNum}\">${dealerEmoji}</span>`
      : ""
  }`;
  const parts = [];
  ...
  return `
    ...
    <span class="tabular-nums text-right justify-self-end flex-none w-[4ch]">${
      pdata.roundScore
    }</span>
    <span class="text-gray-600 truncate min-w-0 flex-1 sm:flex-none sm:block"
          title="${
            pdata.challengeDeductions || pdata.bonus
              ? "(" + breakdown + ")"
              : ""
          }">
...
function renderPlayerRow(
  ...
  player,
  pdata,
  { interactive = true, expectedCards = null, round = null } = {}
) {
  ...
  const hasSubmitted = !!(
    round &&
    round.submittedPlayers &&
    round.submittedPlayers[player]
  );
  ...
  if (isNoSubmission) {
    ...
    if (window.QuiddlerValidation?.validatePlayerWords) {
      const res = window.QuiddlerValidation.validatePlayerWords(raw, {
        expectedCards,
      });
  ...
  const valHTML = (() => {
    ...
    return `<span class="text-red-600 text-xs cursor-help row-val-flag" data-tippy-content="${htmlPieces.join(
      "<br/>"
    )}" title="">🚩</span>`;
  })();

  const controls = interactive
    ? renderRowControls(roundIdx, player, valHTML)
    : "";
  ...
  <input type="text" class="border rounded px-2 py-1 flex-auto min-w-0 w-full sm:w-auto text-left edit-input" data-expected-cards="${
    round.roundNum
  }"
...
function renderRound(
  round,
  roundIdx,
  { interactive = true, topBorder = false } = {}
) {
  ...
  const rows = playerList
    .map((player) =>
      renderPlayerRow(roundIdx, player, round.players[player], {
        interactive,
        expectedCards: round.roundNum,
        round,
      })
    )
    .join("");
...
function initChitTooltips(container = document) {
  ...
  const breakdownInstances = tippy(
    container.querySelectorAll(".breakdown-tip"),
    {
      delay: [100, 50],
      animation: "scale",
      allowHTML: true,
  ...
  const dealerInstances = tippy(
    container.querySelectorAll(".dealer-indicator"),
    {
      delay: [500, 0],
      animation: "none",
      placement: "bottom",
...
function renderOptimizedPlayFromResult(containerId, result) {
  ...
  if (Array.isArray(result.unusedTiles) && result.unusedTiles.length) {
    ...
    wordRows.push(
      `<div class="flex justify-center">${renderChit(
  ...
  if (result.discardTile) {
    ...
    const discardScore = calculateScore(
      parseCards(discardText.replace("-", ""))
    );
    wordRows.push(
      `<div class="flex justify-center">${renderChit(
        { text: discardText, score: discardScore, state: "neutral" },
        {
          ...
          forceState: "neutral",
          showDefIcon: false,
          extraClasses: "bg-yellow-200 !m-0",
        }
      )}</div>`
    );
  }
  ...
  el.innerHTML = `
    <div class="space-y-2 text-center text-sm">
      ...
      <div class="flex flex-col gap-2 items-center mt-2">${wordRows.join(
        ""
      )}</div>
    </div>
  `;
